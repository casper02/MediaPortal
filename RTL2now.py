...
class RTL2nowFilmeListeScreen(Screen):
	...
	def keyOK(self):
		...
		if sperre == "22":
			message = self.session.open(MessageBox, _("Dieses Video ist aus Jugenschutzgruenden momentan gesperrt und ist erst ab ca. 22 Uhr verfuegbar."), MessageBox.TYPE_INFO, timeout=5)
			return
		if sperre == "23":
			message = self.session.open(MessageBox, _("Dieses Video ist aus Jugenschutzgruenden momentan gesperrt und ist erst ab ca. 23 Uhr verfuegbar."), MessageBox.TYPE_INFO, timeout=5)
			return
		print self.pageurl
