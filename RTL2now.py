...
class rtl2SerieScreen(Screen):
	...
	def loadPageData(self, data):
		...
		if selects:
			tabSelects = "&xajaxargs[]="+selects.group(1)+"&xajaxargs[]="+selects.group(2)+"&xajaxargs[]="+selects.group(3)+"&xajaxargs[]="+selects.group(4)+"&xajaxargs[]="+selects.group(5)+"&xajax=show_top_and_movies&xajaxr="+str(time.time()).replace('.','')
			...
			if len(ajax_posts) != 0:
				...
				downloads = [ds.run(self.download,item).addCallback(self.get_series).addErrback(self.dataError) for item in ajax_posts]
				finished = defer.DeferredList(downloads).addErrback(self.dataError)
				
				#addData = postPage(ajaxUrl,"xajaxargs[]="+tab+tabSelects)
				#allData += addData
